        ...
        <div class="ui-box">
          {#if labelingsEnabled}
            ...
            <Input type="checkbox" bind:value={product.global_full_margin}>
              Odgórna marża na całość ({$globalMargins.full_margin}%, {$globalMargins.full_minimum}zł)
            </Input>
            {#if !product.global_full_margin}
              <div class="ui-box ui-box--optional">
                <div class="ui-pair">
                  <Input type="number" bind:value={product.full_margin}>Marża</Input>
                  <Input type="number" bind:value={product.full_minimum}>Minimum</Input>
                </div>
              </div>
            {/if}

            <Input type="checkbox" bind:value={product.global_product_margin}>
              Odgórna marża na produkt ({$globalMargins.product_margin}%, {$globalMargins.product_minimum}zł)
            </Input>
            {#if !product.global_product_margin}
              <div class="ui-box ui-box--optional">
                <div class="ui-pair">
                  <Input type="number" bind:value={product.product_margin}>Marża</Input>
                  <Input type="number" bind:value={product.product_minimum}>Minimum</Input>
                </div>
              </div>
            {/if}
          {:else}
      ...
      <div class="ui-section__col labelings">
        <div class="ui-box">
          ...
          <div class="ui-section__row">
            {#each product.labelings as labeling, i (labeling)}
              ...
              >
                <div class="ui-pair">
                  ...
                  <Button icon={'delete.svg'} on:click={() => removeLabeling(i)} dangerous>Usuń</Button>
                </div>
                ...
                {#if chosenLabeling && labeling.enabled}
                  ...
                  {#if !labeling.global_margin}
                    <div class="ui-box ui-box--optional">
                      <div class="ui-pair">
                        <Input type="number" bind:value={labeling.margin}>Marża</Input>
                        <Input type="number" bind:value={labeling.minimum}>Minimum</Input>
                      </div>
                    </div>
                  {/if}
                {/if}
            ...
            <Button icon={'add.svg'} on:click={pushLabeling}>Dodaj</Button>
          </div>
        </div>
      </div>
    </div>
